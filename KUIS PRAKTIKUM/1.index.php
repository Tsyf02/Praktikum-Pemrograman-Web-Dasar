<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <meta name="author" content="Talitha Syifa Al Fath_124250173">
    <meta name="description" content="web PokeMatch">
    <title> PokeMatch </title>
    <link rel="icon" href="data:image/png;base64," type="image/x-icon">
    <link rel="stylesheet" href="stylesheet.css" type="text/css">
    <link href="" rel="stylesheet"
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;900&family=DM+Sans:wght@300;400;500&display=swap" rel="stylesheet"/>
</head>
<body>
    <nav>
    <a href="login.php" class="logo">PokeMatch</a>
    <ul class="nav-links">
        <li><a href="1.index.php">Home</a></li>
        <li><a href="1.index.php">Kategori</a></li>
        <li><a href="2.form.php" class="active">Pendaftaran</a></li>
</ul>
</nav>

<center>
<div>
<container> 
    <img src="https://www.google.com/imgres?q=logo%20pokemon&imgurl=https%3A%2F%2Fimages.template.net%2F494381%2FPokemon-Logo-Clipart-edit-online.png&imgrefurl=https%3A%2F%2Fwww.template.net%2Fedit-online%2F494381%2Fpokemon-logo-clipart&docid=PIMKvkzn9tiNAM&tbnid=5BCQrUyW0uzrxM&vet=12ahUKEwjCx7HPgMeTAxXQ1zgGHWAkF1oQnPAOegQIJhAB..i&w=1200&h=1200&hcb=2&ved=2ahUKEwjCx7HPgMeTAxXQ1zgGHWAkF1oQnPAOegQIJhAB" alt="gambarpokemon">
        <h1> PokeMatch </h1>
<p>Sebuah platform bernama PokeMatch dibuat untuk memudahkan para pemain
Pokémon dalam mengikuti berbagai turnamen. Pada halaman utama, sistem menampilkan
beberapa kategori turnamen yang tersedia </p>
<br>
<a href="2.form.php" class="btn-primary mb-4">Cari Turnamen Sekarang </a>
</container>
</div></center>
<br>

<center> <div>
    <div class="about-details">
        <div class="detail-item">
            <div class="book-card">
                <div class="book-card-body">
                    <h3>Pikachu Cup </h3>
                    <p>- Level trainer minimal 5</p> <br>
                    <p>- Memiliki minimal 1 Pokemon</p> <br>
                    <center><p>Rp100.000</center></p>
                    <a href="#" class="btn-detail">Daftar Sekarang</a>
                    </div>
                </div>
            </div>

        <div class="detail-item">
            <div class="book-card">
                <div class="book-card-body">
                    <h3>Charizard Clash</h3>
                    <p>- Level trainer minimal 10</p> <br>
                    <p>- Memiliki minimal 2 Pokemon</p> <br>
                    <p>- Pernah mengikuti battle</p> <br>
                    <center><p>Rp100.000</center></p>
                    <a href="#" class="btn-detail">Daftar Sekarang</a>
                    </div>
                </div>
            </div>

        <div class="detail-item">
            <div class="book-card">
                <div class="book-card-body">
                    <h3>Mewtwo Master League</h3>
                    <p>- Level trainer minimal 15</p> <br>
                    <p>- Memiliki minimal 3 Pokemon</p> <br>
                    <p>- Memiliki item battle</p> <br>
                            <p><center>Rp150.000</center></p>
                    <a href="#" class="btn-detail">Daftar Sekarang</a>
                    </div>
                </div>
            </div>

        <div class="detail-item">
            <div class="book-card">
                <div class="book-card-body">
                    <h3>Sword & Shield Match</h3>
                    <p>- Deck pokemon dari sword & shield</p> <br>
                    <p>- Percaya Diri untuk Menang</p> <br>
                        <center><p>Rp75.000</center></p>
                    <a href="#" class=".pink-btn">Daftar Sekarang</a>
                    </div>
                </div>
            </div>
</div> </center> 
<br>




<hr>
<center> 
<!-- Remove the container if you want to extend the Footer to full width. -->
<div class="container my-5">

  <footer class="bg-light text-center">
  <!-- Grid container -->
  <div class="container p-4 pb-0">
    <!-- Section: Form -->
    <section class="">
      <form action="">
        <!--Grid row-->
        <div class="row d-flex justify-content-center">
          <!--Grid column-->
          <div class="col-auto">
            <p class="pt-2">
              <strong>Sign up for our newsletter</strong>
            </p>
          </div>
          <!--Grid column-->

          <!--Grid column-->
          <div class="col-md-5 col-12">
            <!-- Email input -->
            <div class="form-outline mb-4">
              <input type="email" id="form5Example2" class="form-control" />
              <label class="form-label" for="form5Example2">Email address</label>
            </div>
          </div>
          <!--Grid column-->

          <!--Grid column-->
          <div class="col-auto">
            <!-- Submit button -->
            <button type="submit" class="btn btn-primary mb-4">
              Subscribe
            </button>
          </div>
          <!--Grid column-->
        </div>
        <!--Grid row-->
      </form>
    </section>
    <!-- Section: Form -->
  </div>
  <!-- Grid container -->

  <!-- Copyright -->
  <div class="text-center p-3" style="background-color: rgba(0, 0, 0, 0.2);">
    © 2020 Copyright: PokeMatch
  </div>
  <!-- Copyright -->
</footer>
  
</div>
<!-- End of .container -->
</center> 

</body>
</html>
